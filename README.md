...
- Chat-completion generation repeats that exact canvas through the wrapper's top-level `image_config` and inside `image_generation`, including `width`, `height`, `size`, and aspect ratio. NAI Diffusion V5 formats `size` with a colon (for example `832:1216`), while V4.5 and other models retain the existing `832x1216` format. This handles proxies that ignore provider-specific nested NAI parameters and otherwise fall back to a default landscape canvas. Values are duplicated unchanged; they are never reversed.
