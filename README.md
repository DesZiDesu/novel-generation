...
- RVL's normal NAI Diffusion V5 and V4.5 models use `/v1/chat/completions` with the compact top-level fields `size`, `width`, `height`, and `seed`; their `size` uses a colon (for example `832:1216`). A model explicitly prefixed with `[备用]` uses `/v1/images/generations` and retains the existing `832x1216` format. Other providers keep the existing configurable route fallback behavior.
