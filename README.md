...
- RVL's normal NAI Diffusion V5 and V4.5 models use `/v1/chat/completions` with one compact top-level payload containing `size`, `width`, `height`, and `seed`; their `size` uses a colon (for example `832:1216`). When Seed is set to random (`-1`), the extension generates a positive six-digit request seed because RVL accepts an unsigned integer. If V5 returns RVL's temporary upstream errors (`502`, `530`, or `bad_response_status_code`), the same request is retried once with `nai-diffusion-4-5-full` and the same seed. A model explicitly prefixed with `[备用]` uses `/v1/images/generations` and retains the existing `832x1216` format. Other providers keep the existing configurable route fallback behavior.
