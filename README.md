...
- RVL's normal NAI Diffusion V5 and V4.5 models use `/v1/chat/completions` with one compact top-level payload containing `size`, `width`, `height`, and `seed`; their `size` uses a colon (for example `832:1216`). When Seed is set to random (`-1`), the extension generates a positive six-digit request seed because RVL accepts an unsigned integer. A selected V5 model remains V5: upstream errors are shown to the user and never retried with V4.5. A model explicitly prefixed with `[备用]` uses `/v1/images/generations` and retains the existing `832x1216` format. Other providers keep the existing configurable route fallback behavior.
